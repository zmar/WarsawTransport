...
cost_of_parking <- function(parking_time) {
  ...
  if (length(parking_time) != 1) {
    stop(
      "Error: argument parking_time is not numeric. Please enter parking_time as numeric vectors of length 1."
    )
  }
  ...
  if (parking_time > 24 | parking_time < 0) {
    stop(
      "Error: argument parking_time is smaller than 0 or greater than 24. Please enter numeric from range [0,24]."
    )
  }
  ...
  if (length(tariff) < (hours + ceiling(minutes))) {
    ...
    tariff <-
      c(tariff, rep(tariff[4], hours + ceiling(minutes) - length(tariff)))
  } else {
  ...
  if (minutes > 0) {
    # if minutes > 0 - calculating cost of each hour of parking - tariff vector multiplied by vector reprezenting each full hour by 1 and last started hour by object 'minutes'
    cost <- tariff * c(rep(1, hours), minutes)
  } else {
    # if minutes <=0 - calculating cost of each hour of parking - tariff vector multiplied by vector reprezenting each full hour by 1
